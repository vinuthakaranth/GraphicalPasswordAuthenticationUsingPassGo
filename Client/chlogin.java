...
class chlpaintPanel extends JPanel implements ChangeListener,MouseMotionListener,MouseListener
{
 

  public chlpaintPanel(JFrame frm)
  {
  ...
  frm=this.frm;
   ...
   add(ig=new JLabel("Choose Image"));
   add(con=new JRadioButton("Conceal",true));
   add(rev= new JRadioButton("Reveal", false));
   ...
   bt.addActionListener(new ActionListener() {
	...
	//System.out.println("checking" + strt);
		String status="";
		try {
	...
	if(status.equals("ok"))
	{
				JOptionPane.showMessageDialog(null,"Your password is correct!! Now you can change password" );
      ...
      repaint();
					...
					if(s.size() >0) {
							...
							String st=(String)s.peek();
                  				if(st.equals("0"))			 //points
                   				...
                   				if(s.size() ==0) {	slidex.enable();
                   					SwingUtilities.updateComponentTreeUI(chlpaintPanel.this);
                   				}
                   				repaint();
					}   			
			...
			add(slidex=new JSlider());
				...
				bt.setBounds(500,500,100,30);
				uname.setBounds(500,410,200,50);
				tbx.setBounds(470,450,150,30);
				pass.setBounds(50,10,150,50);

				clear.setBounds(500,50,100,30);
				Undo.setBounds(500,100,100,30);
		  
                ig.setBounds(500,250,100,60);
                con.setBounds(500,150,100,30);
                    rev.setBounds(500,190,100,30);
    ...
    public boolean isSensitive(int x,int y) {
    	...
    	for(int i=0;i<=scale;i++) {
    		for(int j=0;j<=scale;j++) {
    			...
    			int agx = (j*grid) + 50;
    			int agy = (i*grid) + 50;
    			double d =   Math.sqrt(Math.pow(agx-x,2) + Math.pow(agy-y, 2));
					slidex.disable();
...
JSlider slidex;
 JLabel xs;
 int sca,cval=0;
 JButton bt,clear,Undo,sc,sd,sb,spen,skoala,sx,cp;
 JLabel uname,pass,ig;
 public static JTextField tbx;
 String iname;
 boolean flag=false,flagr=false;
 int mx2,my2,mx1,my1,mc1,mr1;
 Image img,img1,img2,img3;
 boolean first=true;
 ButtonGroup bg=new ButtonGroup();
 int bestx,besty,adjx,adjy;
 JRadioButton con,rev;
